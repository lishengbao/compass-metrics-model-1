...
class DeveloperAttritionMetricsModel(MetricsModel):
    ...
    def get_freq_contributor_attrition_dict(self, from_date, to_date, date_field_contributor_dict,
        ...
        def get_conversion_to_silence_dict():
            ...
            attrition_core_set = self.get_conversion_to_silence_set(from_date, to_date,
            ...
            attrition_total_dict = {
                ...
                "ratio": round(len(attrition_total_set) / len(attraction_total_set), 4) if len(
                    attraction_total_set) > 0 else 0
            }
            attrition_casual_dict = {
                "count": len(attrition_casual_set),
                "ratio": round(len(attrition_casual_set) / len(attraction_casual_set), 4) if len(
                    attraction_casual_set) > 0 else 0
            }
            attrition_regular_dict = {
                "count": len(attrition_regular_set),
                "ratio": round(len(attrition_regular_set) / len(attraction_regular_set), 4) if len(
                    attraction_regular_set) > 0 else 0
            }
            attrition_core_dict = {
                "count": len(attrition_core_set),
                "ratio": round(len(attrition_core_set) / len(attraction_core_set), 4) if len(
                    attraction_core_set) > 0 else 0
            }
        ...
        def get_silence_dict():
            ...
            activity_core_set = self.get_freq_contributor_activity_set(from_date, to_date,
                                                                       date_field_contributor_dict, is_bot, period)
            period_dict = self.period_contribution_last_date_dict.get(period, {})
            period_dict.update({item: "casual" for item in activity_casual_set})
            period_dict.update({item: "regular" for item in activity_regular_set})
            period_dict.update({item: "core" for item in activity_core_set})
            self.period_contribution_last_date_dict[period] = period_dict

            contributor_date_dict = self.get_contribution_activity_date_dict(date_field_contributor_silence_dict,
                                                                             is_bot)
            for contributor_name, contribution_date_list in contributor_date_dict.items():
                if len(list_sub(contribution_date_list, (to_date + str_to_offset("-90d")).isoformat(),
                                to_date.isoformat())) == 0 and contributor_name in period_dict:
                    attrition_total_set.add(contributor_name)
                    if period_dict[contributor_name] == 'casual':
                        attrition_casual_set.add(contributor_name)
                    elif period_dict[contributor_name] == 'regular':
                        attrition_regular_set.add(contributor_name)
                    elif period_dict[contributor_name] == 'core':
        ...
        def get_same_period_conversion_to_silence_list(current_attrition_total_set):
            ...
            if len(period_dict) > 0:
                ...
                for date_key, date_value in sorted_period_items:
                    if len(attrition_total_list) > 48:
                        break
    ...
    def get_type_contributor_attrition_dict(self, from_date, to_date, date_field_contributor_dict, is_bot=False,
        ...
        attrition_casual_set, \
        attrition_regular_set, \
        attrition_core_set = self.get_freq_contributor_attrition_set(from_date, to_date,
                                                                     date_field_contributor_dict, is_bot, period)

        attraction_total_set = self.get_type_contributor_attraction_set(from_date, to_date,
