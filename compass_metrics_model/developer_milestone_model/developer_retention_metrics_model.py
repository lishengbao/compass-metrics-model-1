...
WEIGHTS_FILE = "developer_milestone_model/resources/developer_retention_weights.yaml"
...
class DeveloperRetentionMetricsModel(MetricsModel):
    ...
    def get_freq_contributor_retention_dict(self, from_date, to_date, activity_contributor_date_dict, observe_activity_contributor_date_dict,
                                            attraction_contributor_date_dict, observe_attraction_contributor_date_dict,
                                            period="week"):
        ...
        def get_last_period_retention_dict():
            ...
            last_period_activity_core_set = self.get_freq_contributor_activity_set(
                last_period_from_date, last_period_to_date, activity_contributor_date_dict, observe_activity_contributor_date_dict, period)
        ...
        def get_same_period_retention_list():
            ...
            if len(period_dict) > 0:
                ...
                for date_key, date_value in sorted_period_items:
                    if len(retention_total_list) > 48:
                        break
                    end_date = (datetime_to_utc(str_to_datetime(date_key)) + str_to_offset(self.period_dict[period]["offset"]) + str_to_offset("-1d")).strftime("%Y-%m-%d")
                    ...
                    retention_total_list.append({
                        ...
                        "ratio": round(
                            len(date_value["attraction_total_set"] & current_period_activity_total_set) / len(date_value[
                    ...
                    retention_casual_list.append({
                        ...
                        "ratio": round(
                            len(date_value["attraction_casual_set"] & current_period_activity_total_set) / len(date_value[
                                "attraction_casual_set"]), 4) if len(date_value["attraction_casual_set"]) > 0 else 0,
                        f"{period}_num": period_num
                    })
                    retention_regular_list.append({
                        ...
                        "ratio": round(
                            len(date_value["attraction_regular_set"] & current_period_activity_total_set) / len(date_value[
                                "attraction_regular_set"]), 4) if len(date_value["attraction_regular_set"]) > 0 else 0,
                        f"{period}_num": period_num
                    })
                    retention_core_list.append({
                        ...
                        "ratio": round(
                            len(date_value["attraction_core_set"] & current_period_activity_total_set) / len(date_value[
                                "attraction_core_set"]), 4) if len(date_value["attraction_core_set"]) > 0 else 0,
                        f"{period}_num": period_num
                    })
            ...
            attraction_core_set = self.get_freq_contributor_attraction_set(from_date, to_date,
                                                                           attraction_contributor_date_dict,observe_attraction_contributor_date_dict, period)
    ...
    def get_type_contributor_retention_dict(self, from_date, to_date, contributor_list, date_field_list, is_bot=False,
                                            period="week"):
        ...
        current_period_activity_set = self.get_type_contributor_activity_set(
            from_date, to_date, contributor_list, date_field_list, is_bot)

        last_period_from_date = from_date - str_to_offset(self.period_dict[period]["offset"])
        last_period_to_date = from_date
        last_period_activity_set = self.get_type_contributor_activity_set(
            last_period_from_date, last_period_to_date, contributor_list, date_field_list, is_bot)
        retention_dict = {
        ...
        for period_key, period_value in self.period_dict.items():
            ...
            for date in date_list:
                ...
                total_retention_dict, freq_casual_dict, freq_regular_dict, freq_core_dict = self.get_freq_contributor_retention_dict(
                    date,
                    to_date,
                    contribution_activity_date_dict,
                    observe_contribution_activity_date_dict,
                    contribution_attraction_date_dict,
                    observe_contribution_attraction_date_dict,
                    period=period_key)
                ...
                type_observe_star_dict = self.get_type_contributor_retention_dict(date, to_date, star_contributor_list,
                                                                                  ["star_date_list"], period=period_key)
                type_observe_fork_dict = self.get_type_contributor_retention_dict(date, to_date, fork_contributor_list,
                                                                                  ["fork_date_list"], period=period_key)
